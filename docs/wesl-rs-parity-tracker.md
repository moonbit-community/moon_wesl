...
| PARITY-012 | `wesl-rs` broader test and conformance posture, local `wesl_test.mbt` | Build a much stronger parity and regression test suite. | `IN_PROGRESS` | Phase 1. On 2026-04-15 the repo gained a vendored upstream corpus under `testdata/upstream/` plus a whitebox harness in `upstream_wbtest.mbt`, `upstream_cases.mbt`, and `upstream_test_support.mbt`. The imported suites now cover spec syntax fixtures, import/context/module fixtures, conditional translation fixtures, dead-code fixtures, bulk-test metadata, ctor-coverage metadata, and e2e syntax smoke coverage for vendored Bevy and WGPU shaders. The newest parity slice no longer just vendors those cases: it closes the entire currently tracked upstream import-syntax pending list and the entire currently tracked upstream import-compilation pending list. Concretely, `moon_wesl` now accepts grouped and bare module imports such as `import foo;`, rejects empty imports such as `import;`, resolves inline `package::...` / `super::...` qualified references, supports namespace-style imports like `import super::file1;`, preserves local shadowing after imported references, and keeps upstream-derived regression coverage active instead of skipped. Verification: `moon check --target all` and `moon test -v` both pass locally on 2026-04-15, with the expanded suite still 25/25 green after removing those import pending lists. Remaining explicit pending areas are now concentrated in higher-order parity gaps such as conditional translation breadth and circular validation semantics. Exit criteria: keep shrinking the remaining pending upstream-case lists, add stronger semantic validation/e2e execution beyond current syntax-smoke coverage, and keep each parity issue landing with corresponding upstream-derived regression coverage. |
